...
class DatabaseModel: NSObject {
    ...
    func demSoPhongCoDieuKien(idNguoiTao: Int) -> NSMutableArray {
        sharedInstance.database!.open()
        let resultSet:FMResultSet! = sharedInstance.database!.executeQuery("SELECT * FROM phong WHERE id_nguoitao = ?", withArgumentsIn: [idNguoiTao])
    ...
    func themPhong(_ PhongModel: PhongModel) -> Bool {
        sharedInstance.database!.open()
        let isInserted = sharedInstance.database!.executeUpdate("INSERT INTO phong (tenphong, trangthai, songuoi, ngaydat, tienphong, ngaythanhtoan, id_nguoitao) VALUES(?, ?, ?, ?, ?, ?, ?)", withArgumentsIn: [PhongModel.tenphong!, PhongModel.trangthai!, PhongModel.songuoi!, PhongModel.ngaydat!, PhongModel.tienphong!, PhongModel.ngaythanhtoan!, PhongModel.id_nguoitao!])
    ...
    func suaThongTinPhong(tenPhong: String, ngayDat: String, tienPhong: Int, soNguoi: Int, ngayThanhToan: String, idPhong: Int) -> Bool {
        sharedInstance.database!.open()

        let isUpdated = sharedInstance.database!.executeUpdate("UPDATE PHONG SET tenphong = ?, ngaydat = ?, tienphong = ?, songuoi = ?, ngaythanhtoan = ? WHERE id = ?", withArgumentsIn: [tenPhong, ngayDat, tienPhong, soNguoi, ngayThanhToan, idPhong])
